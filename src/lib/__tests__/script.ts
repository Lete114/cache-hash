...
import handlerScript from '../script'
...
describe('handlerScript', () => {
  ...
  it('hash injection', () => {
    const options: handlerType = {
      options: { output: TEST_DIR, versionKey: 'v', script: true },
      cssFiles: [],
      jsFiles: [],
    ...
    handlerScript(options)

    const mainCssContent = fs.readFileSync(indexHtml, { encoding: 'utf8' })

    const str = 'index.js?name=aaa&v=fc3ff98e8c'

...
import handlerStyle from '../style'
...
describe('handlerStyle', () => {
  let TEST_DIR: string

  beforeEach((done) => {
    TEST_DIR = join(tmpdir(), 'cache-hash', 'handlerStyle')
  ...
  it('hash injection', () => {
    const options: handlerType = {
      options: { output: TEST_DIR, versionKey: 'v', style: true },
      cssFiles: [],
      jsFiles: [],
